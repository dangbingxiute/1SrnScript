            ...
            for i,l in pairs(game:GetService("Workspace").Game.Entities.ItemPickup:GetChildren()) do
                for i,v in pairs(l:GetChildren()) do
                    if v.ClassName == "MeshPart" or "Part" then
                        for i,e in pairs(v:GetChildren()) do
                            if e.ClassName == "ProximityPrompt" then
                                if e.ObjectText == "Large Present" or e.ObjectText == "Medium Present" then                                     local epoh1 = v.CFrame
                                    epoh3.CFrame = epoh1 * CFrame.new(0, 2, 0)
                                    wait(0.1)
                                    e:InputHoldBegin()
                                    e:InputHoldEnd()
        ...
        local autoPresent = makeutilbutton("自动捡小礼物盒")
        local autoPresentt = false
        autoPresent.MouseButton1Click:Connect(function()
            if autoPresentt == false then
                autoPresentt = true
                autoPresent.Text = "自动捡小礼物盒(开)"
            else
                autoPresentt = false
                autoPresent.Text = "自动捡小礼物盒"
            end
            while autoPresentt == true do
        	wait(0.1)
            local epoh2 = game:GetService("Players")
            ...
            for i,l in pairs(game:GetService("Workspace").Game.Entities.ItemPickup:GetChildren()) do
                for i,v in pairs(l:GetChildren()) do
                    if v.ClassName == "MeshPart" or "Part" then
                        for i,e in pairs(v:GetChildren()) do
                            if e.ClassName == "ProximityPrompt" then
                                if e.ObjectText == "Small Present" then                                        local epoh1 = v.CFrame
                                    epoh3.CFrame = epoh1 * CFrame.new(0, 2, 0)
                                    wait(0.1)
                                    e:InputHoldBegin()
                                    e:InputHoldEnd()
        ...
        game:GetService("Players").LocalPlayer.PlayerGui.Money.Container.premium.TextLabel.Text = "Escape User"
        game:GetService("Players").LocalPlayer.PlayerGui.Money.Container["2x cash"].Visible = true
        game:GetService("Players").LocalPlayer.PlayerGui.Money.Container["2x cash"].TextLabel.Text = "Escape By 1Srn"
        ...
        game:GetService("Workspace").OhioSign.Screen.SurfaceGui.Frame.Population.Text = "Escape User"
        
        local Teams = game:GetService("Teams")
        local newTeam = Instance.new("Team")
        newTeam.Name = "Escape User"
